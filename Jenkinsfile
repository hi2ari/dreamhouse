...
*/
            ...
            stage('Display Install Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}\" force:org:display --targetusername ${SF_Install_Package_Scratch_Org_Username}"
                if (rc != 0) {
            ...
            stage('Install Package In Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}\" force:package:install --package ${PACKAGE_VERSION} --targetusername ${SF_Install_Package_Scratch_Org_Username} --wait 10 --publishwait 10 --noprompt"
                if (rc != 0) {
            ...
            stage('Generate Password for Package Install Scratch Org User') {
                rmsg = bat returnStdout: true, script: "\"${toolbelt}\" force:user:password:generate --targetusername ${SF_Install_Package_Scratch_Org_Username} --json"
                //if (rc != 0) {
            ...
            // -------------------------------------------------------------------------
			    ...
			    stage('Display Details for Package Install Scratch Org User') {
				      rmsg = bat returnStdout: true, script: "\"${toolbelt}\" force:user:display --targetusername ${SF_Install_Package_Scratch_Org_Username} --json"
				      println(rmsg)
				      //println('Hello from a Job DSL script1!')
				      def beginIndex = rmsg.indexOf('{')
				      def endIndex = rmsg.indexOf('}')
